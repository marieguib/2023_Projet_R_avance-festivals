...
ggplot(df_mois, aes(x = Mois, y = Frequence)) +
  ...
  guides(fill=FALSE) 
```
Nous observons que plus les beaux jours arrivent, plus le nombre de festivals augmente. 
...
frequentation_num <- gsub(" ", "", frequentation)
annees <- 2011:2019
df <- data.frame(annees, frequentation_num)
df$frequentation_num <- gsub(" ", "", df$frequentation_num)
df
```

Cepedant, nous pouvons observer que certaines éditions n'ont pas eu lieu à cause de la crise sanitaire.
...
Les logos de l'année 2015 n'est pas disponible sur le site de wikipédia. Cependant, nous avons trouvé ça important de montrer le logo actuel du festival, en place depuis 2016. 

```{r}
library(png)
electrobeach_site <- "https://www.electrobeach.com"
