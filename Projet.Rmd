...
df_festivals <- df_festivals |> 
  ...
  select(-periodicite) # suppression de la colonne periodicite car les informations sont redondantes à présent
...
# Créer un dataframe qui contient les noms des départements et des régions
df_regions_dpt <- df_festivals |> 
  select(nom_departement, region) |> 
  rename(Departement = nom_departement, Region = region)

df_regions_dpt <- left_join(df_regions_dpt,df_regions,by="Region")
df_regions_dpt

ggplot(df_regions_dpt, aes(x = Frequence, y = Region)) +
...
```{r}
liste_dates <- festival_html |> 
  html_nodes(xpath = "//*[@id='post-2934']/div/div/div/p[9]/strong") |> 
  html_text()
liste_dates
# Nous intéresse toutes les 5 valeurs : 4, 9, 14...
```

```{r}
for(fest in listes_festivals){
  cat("Le festival", fest, "aura lieu les", liste_dates, "à", liste_lieux, "\n")
}
```

## Mise en relation avec le JSON
